...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'kale-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('kale').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('kale')[service]);
